...
export default {
  ...
  data () {
    ...
    Object.keys(columnsList).map(key => {
      columnsList[key].width = columnsList[key].width || 150
      if (columnsList[key].form) {
        columnsList[key].form.size = this.dialogSize === 'small' ? 12 : 24
      }
    })
    return {
    ...
    getRelation (value) {
      ...
      .then((data) => {
        ...
        Object.keys(this.columnsList).map(key => {
          const relation = this.columnsList[key].relation || key
          if (data[relation]) {
        ...
        this.$emit('getRelation', this.columnsList, data)
        this.getData()
      }).catch(() => {
        this.isLoading = false
      })
